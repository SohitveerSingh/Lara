<div class="topbar container-fluid">
    <div class="d-flex align-items-center gap-1">
        <div class="logo-topbar"><a class="logo-light" href="https://themes.techzaa.in/velonic_r/"><span
                    class="logo-lg"><img
                        src="./Welcome! _ Velonic React - Bootstrap 5 Admin &amp; Dashboard Template_files/logo-3711f1d8.png"
                        alt="logo" class=""></span><span class="logo-sm"><img
                        src="data:image/png;base64,"
                        alt="small logo" class=""></span></a><a class="logo-dark"
                href="https://themes.techzaa.in/velonic_r/"><span class="logo-lg"><img
                        src="./Welcome! _ Velonic React - Bootstrap 5 Admin &amp; Dashboard Template_files/logo-dark-503b1844.png"
                        alt="dark logo"></span><span class="logo-sm"><img
                        src="data:image/png;base64,"
                        alt="small logo"></span></a></div><button class="button-toggle-menu"><i
                class="ri-menu-line"></i></button><button class="navbar-toggle " data-bs-toggle="collapse"
            data-bs-target="#topnav-menu-content">
            <div class="lines"><span></span><span></span><span></span></div>
        </button>
        <div class="app-search d-none d-lg-block">
            <form>
                <div class="input-group"><input type="search" class="form-control" placeholder="Search..."><span
                        class="ri-search-line search-icon text-muted"></span></div>
            </form>
        </div>
    </div>
    <ul class="topbar-menu d-flex align-items-center gap-3">
        <li class="dropdown d-lg-none">
            <div class="dropdown"><a class="nav-link dropdown-toggle arrow-none dropdown-toggle"
                    id="react-aria9940339260-:r0:" aria-expanded="false" role="button"><i
                        class="ri-search-line fs-22"></i></a></div>
        </li>
        <li class="dropdown">
            <div class="dropdown"><a class="nav-link dropdown-toggle arrow-none dropdown-toggle"
                    id="react-aria9940339260-:r1:" aria-expanded="false" role="button"><img
                        src="data:image/jpeg;base64,"
                        alt="user-avatar" class="me-0 me-sm-1" height="12"><span
                        class="align-middle d-none d-lg-inline-block">English</span><i
                        class="ri-arrow-down-s-line d-none d-sm-inline-block align-middle"></i></a></div>
        </li>
        <li class="d-none d-sm-inline-block">
            <div class="nav-link" id="light-dark-mode"><i class="ri-moon-line fs-22"></i></div>
        </li>
        <li class="dropdown">
            <div class="dropdown"><a class="nav-link dropdown-toggle arrow-none nav-user dropdown-toggle"
                    id="react-aria9940339260-:r4:" aria-expanded="false" role="button"
                    href=""><span class="account-user-avatar"><img
                            src="./images/Pic1.jpg" alt="user-image" width="32" class="rounded-circle"></span><span
                        class="d-lg-block d-none">
                        <h5 class="my-0 fw-normal">Thomson <i
                                class="ri-arrow-down-s-line d-none d-sm-inline-block align-middle"></i></h5>
                    </span></a></div>
        </li>
    </ul>
</div>