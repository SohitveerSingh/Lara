<div class="topbar container-fluid">
  <div class="d-flex align-items-center gap-1">
    <div class="logo-topbar">
      <a class="logo-light" href="https://themes.techzaa.in/velonic_r/"><span class="logo-lg"><img
            src="./Welcome! _ Velonic React - Bootstrap 5 Admin &amp; Dashboard Template_files/logo-3711f1d8.png"
            alt="logo" class="" /></span><span class="logo-sm"><img
            src="data:image/png;base64,"
            alt="small logo" class="" /></span></a><a class="logo-dark"
        href="https://themes.techzaa.in/velonic_r/"><span class="logo-lg"><img
            src="./Welcome! _ Velonic React - Bootstrap 5 Admin &amp; Dashboard Template_files/logo-dark-503b1844.png"
            alt="dark logo" /></span><span class="logo-sm"><img
            src="data:image/png;base64,"
            alt="small logo" /></span></a>
    </div>
    <button class="button-toggle-menu">
      <i class="ri-menu-line"></i></button><button class="navbar-toggle" data-bs-toggle="collapse"
      data-bs-target="#topnav-menu-content">
      <div class="lines"><span></span><span></span><span></span></div>
    </button>
    <div class="app-search d-none d-lg-block">
      <form>
        <div class="input-group">
          <input type="search" class="form-control" placeholder="Search..." /><span
            class="ri-search-line search-icon text-muted"></span>
        </div>
      </form>
    </div>
  </div>
  <ul class="topbar-menu d-flex align-items-center gap-3">
    <li class="dropdown d-lg-none">
      <div class="dropdown">
        <a class="nav-link dropdown-toggle arrow-none dropdown-toggle" id="react-aria5046092502-:r0:"
          aria-expanded="false" role="button"><i class="ri-search-line fs-22"></i></a>
      </div>
    </li>
    <li class="dropdown">
      <div class="dropdown">
        <a class="nav-link dropdown-toggle arrow-none dropdown-toggle" id="react-aria5046092502-:r1:"
          aria-expanded="false" role="button"><img
            src="data:image/jpeg;base64,"
            alt="user-avatar" class="me-0 me-sm-1" height="12" /><span
            class="align-middle d-none d-lg-inline-block">English</span><i
            class="ri-arrow-down-s-line d-none d-sm-inline-block align-middle"></i></a>
      </div>
    </li>



    <li class="d-none d-sm-inline-block">
      <div class="nav-link" id="light-dark-mode">
        <i class="ri-moon-line fs-22"></i>
      </div>
    </li>
    <li class="dropdown">
      <div class="dropdown">
        <a class="nav-link dropdown-toggle arrow-none nav-user dropdown-toggle" id="react-aria5046092502-:r4:"
          aria-expanded="false" role="button" href="#"><span class="account-user-avatar"><img
              src="./Welcome! _ Velonic React - Bootstrap 5 Admin &amp; Dashboard Template_files/avatar-1-d6485790.jpg"
              alt="user-image" width="32" class="rounded-circle" /></span><span class="d-lg-block d-none">
            <h5 class="my-0 fw-normal">
              Thomson
              <i class="ri-arrow-down-s-line d-none d-sm-inline-block align-middle"></i>
            </h5>
          </span></a>
        <!-- <div x-placement="bottom-end" aria-labelledby="react-aria2662280715-:r4:"
          class="dropdown-menu-animated profile-dropdown dropdown-menu show dropdown-menu-end"
          data-popper-reference-hidden="false" data-popper-escaped="false" data-popper-placement="bottom-end"
          style="position: absolute; inset: 0px 0px auto auto; transform: translate(0px, 72.5px);">
          <div>
            <div class=" dropdown-header noti-title">
              <h6 class="text-overflow m-0">Welcome !</h6>
            </div>
            <a class="dropdown-item" href="/velonic_r/pages/profile">
              <i class="ri-account-circle-line fs-18 align-middle me-1"></i>
              <span>My Account</span>
            </a>
            <a class="dropdown-item" href="/velonic_r/pages/profile">
              <i class="ri-settings-4-line fs-18 align-middle me-1">
              </i>
              <span>Settings</span>
            </a>
            <a class="dropdown-item" href="/velonic_r/pages/faq">
              <i class="ri-customer-service-2-line fs-18 align-middle me-1"></i>
              <span>Support</span>
            </a><a class="dropdown-item" href="/velonic_r/auth/lock-screen">
              <i class="ri-lock-password-line fs-18 align-middle me-1"></i>
              <span>Lock Screen</span></a>
            <a class="dropdown-item" href="/velonic_r/auth/logout">
              <i class="ri-logout-box-line fs-18 align-middle me-1">
              </i>
              <span>Logout</span>
            </a>
          </div>
        </div> -->
      </div>
    </li>
  </ul>
</div>